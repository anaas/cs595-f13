...
def loadMovieLens():
  ...
  cumulative_simi={}
  for user in prefs.keys():
   simi.setdefault(user,0)
   cumulative_simi.setdefault(user,0)
   simi[user]=topMatches(prefs,user)[0 : 4 ]
   num=0
   raters=[]
   for (key,value) in simi[user]:
    num+=key
    raters.append(value)
   cumulative_simi[user]=(num,raters)
  ...
  cumulative={}
  for user in prefs.keys():
   diff.setdefault(user,0)
   cumulative.setdefault(user,0)
   diff[user]=topMatches(prefs,user)[len(topMatches(prefs,user)) - 5 : len(topMatches(prefs,user)) - 1 ]
   num=0
   raters=[]
   for (key,value) in diff[user]:
    num+=key
    raters.append(value)
