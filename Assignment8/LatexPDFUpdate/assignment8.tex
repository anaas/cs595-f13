...
Table 1 shows that there are ten ties movies that have the highest average ratings.
...
\begin{itemize}
\item Created a new dictionary called {\it lengthList} and find the length of {\it rating\_hi\_avg} from question one for all rating of each movies.
...
Table 2 shows the movies that received the most ratings.
\begin{table}[h!]
...
Table 3 shows the movies ties that were rated the highest on average by women.
\begin{table}[ht]
...
Table 4 shows the movies ties that were rated the highest on average by men.
...
For answering question five I just invert the preference matrix to be user-centric in {\it prefs} dictionary instead of movie-centric and use the {\it topMatches} function which give the most and least like match to {\it Top Gun}.
...
Table 5 shows the movies that most and least like match to {\it Top Gun} based on the following Pearson's correlation:
...
Table 6 shows the five raters rated the most films.
...
\begin{answer}{7}
In this question I found that there are more than five ties agreed with each other and I computed scores for each user to every other users by calling {\it topMatches} function which return the top fore similarity scores for everyone. 

\lstinputlisting[language=Python, firstline=297, lastline=302]{code/assignment8.py}

 
\begin{itemize}
\item Define a cumulative difference score after sorting, and report the group of five that have the smallest cumulative difference scores.
\end{itemize}
 \lstinputlisting[language=Python, firstline=303, lastline=313]{code/assignment8.py} 


Table 7 shows the five raters most agreed with each other.
\begin{table}[ht]
...
772   &   889, 899, 780, 277 & 0.0  \\
375   &   927, 683, 634, 204 & 0.0  \\
610   &   914, 856, 53, 139 & 0.0  \\
196   &   9, 461, 147, 129 & 0.0  \\
78   &   885, 823, 821, 690 & 0.0  \\
...
\end{answer}
\begin{answer}{8}
In this question I did the same thing as in question seven, the only change was:
\begin{itemize}

\item Reporting the group of five that have the \textcolor {red}{highest} cumulative difference scores.

\lstinputlisting[language=Python, firstline=328, lastline=331]{code/assignment8.py} 

\end{itemize}

Table 8 shows the five raters most disagreed with each other.
\begin{table}[ht]
...
196   &  594, 568, 263, 122 & 8  \\
400   &  65, 545, 467, 325 & 8  \\
260   &  671, 525, 501, 232 & 8  \\
928   &  760, 547, 432, 317 & 8 \\
832   &  766, 622, 613, 491 & 8  \\  
...
\item I sorted all movies based on their average and reverse to get the highest average rating and print the movies with highest average.

\lstinputlisting[language=Python, firstline=354, lastline=358]{code/assignment8.py}
\lstinputlisting[language=Python, firstline=363, lastline=367]{code/assignment8.py}

\end{itemize}

Table 9 shows the movies that were rated highest on average by men under 40.
\begin{table}[ht]
...
Table 10 shows the movies that were rated highest on average by men over 40.
\begin{table}[ht]
...
\lstinputlisting[language=Python, firstline=374, lastline=374]{code/assignment8.py}


Table 11 shows the movies that were rated highest on average by women under 40.
\begin{table}[h!]
\centering
...
Table 12 shows the movies that were rated highest on average by women over 40.
\begin{table}[ht]
\centering
...
\bibitem{source}
 \url {http://www.cs.odu.edu/~mln/teaching/cs595-f13/?method=getElement&element=~assignments~a8~a8.txt}.
\end{thebibliography}
