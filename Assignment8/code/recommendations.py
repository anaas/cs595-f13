...
def calculateSimilarUser(prefs,n=10):
  ...
  # are most similar to.
  result={}
  # Invert the preference matrix to be user-centric
  # itemPrefs = transformPrefs(prefs)
  c=0
  for user in prefs:
    # Status updates for large datasets
...
def loadMovieLens():
  ...
  for user in prefs.keys(): 	
    for key,value in prefs[user].iteritems():
		rating_most.setdefault(key,[])
		rating_most[key].append(value)
  lengthList = {}  
  for movie in rating_most.keys(): 	
    lenlist = len(rating_most[movie])
  ...
  print('Least match : ',topMatches(flip_prefs,'Top Gun (1986)')[len(topMatches(flip_prefs,'Top Gun (1986)')) - 1 ])
  ...
  for user in simi_users:
    ...
    print("----------------- key and value of user's value --------------------------------")
  ...
  print ('Number of ties ---- ',i) 	
	...
	#print type(key),key,'  ...  ',type(value),value
	  ...
	  #cumulative[user][key] += int(key)
	  #cumulative[user][value] += str(value)
  #print (simi_users)
  ### Q7 End ####  
  ...
  sorted_wd.reverse()
  print (" ##### number [10 A] solution ##### ")
  for (key,value) in sorted_wd[0:11]:
   print key,'  ...  ',value
  average_w_up = {}  
  ...
  sorted_wu.reverse()
  print (" ##### number [10 B] solution ##### ")
  for (key,value) in sorted_wu[0:11]:
   print key,'  ...  ',value
  ### Q10 End ####
